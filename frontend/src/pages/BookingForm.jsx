...
const s = {
  ...
  backBtn: { display: "flex", alignItems: "center", gap: "6px", background: "none", border: "none", color: "#7a8099", fontSize: "13px", cursor: "pointer", padding: "0", marginBottom: "1.75rem", fontFamily: "sans-serif" },
  ...
  sectionTitle: { fontSize: "11px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: "#c8a96e", marginBottom: "1.25rem" },
  ...
  input: { background: "#0d1525", border: "0.5px solid #1f2a3c", borderRadius: "8px", padding: "10px 12px", fontSize: "14px", color: "#f0ebe0", outline: "none", fontFamily: "sans-serif", width: "100%", boxSizing: "border-box", transition: "border-color 0.15s" },
  inputError: { borderColor: "#7f1d1d" },
  inputFocus: { borderColor: "#c8a96e" },
  errorMsg: { fontSize: "11px", color: "#f87171", marginTop: "2px" },
  select: { background: "#0d1525", border: "0.5px solid #1f2a3c", borderRadius: "8px", padding: "10px 12px", fontSize: "14px", color: "#f0ebe0", outline: "none", fontFamily: "sans-serif", width: "100%", boxSizing: "border-box" },
  ...
  submitBtn: { width: "100%", background: "#c8a96e", color: "#0a0f1e", border: "none", borderRadius: "8px", padding: "13px", fontSize: "14px", fontWeight: "600", cursor: "pointer", letterSpacing: "0.5px", fontFamily: "sans-serif", marginTop: "1.5rem" },
  ...
  securityNote: { display: "flex", alignItems: "center", gap: "8px", padding: "10px 12px", background: "#0d1a0d", border: "0.5px solid #14291a", borderRadius: "8px", fontSize: "11px", color: "#4ade80", marginTop: "1.25rem" },
  secDot: { width: "5px", height: "5px", borderRadius: "50%", background: "#4ade80", flexShrink: 0 },
  passengerHeader: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" },
  passengerBadge: { fontSize: "11px", color: "#c8a96e", background: "#1a1507", border: "0.5px solid #2a2208", borderRadius: "4px", padding: "3px 8px" },
  successOverlay: { minHeight: "100vh", background: "#0a0f1e", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "1rem", padding: "2rem" },
  successIcon: { width: "64px", height: "64px", background: "#0d1a0d", border: "0.5px solid #14291a", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "1rem" },
  ...
  successBtn: { background: "#c8a96e", color: "#0a0f1e", border: "none", borderRadius: "8px", padding: "12px 28px", fontSize: "14px", fontWeight: "600", cursor: "pointer", fontFamily: "sans-serif", marginTop: "1rem" },
};
...
export default function BookingForm() {
  ...
  const updatePassenger = (index, field, value) => {
    let sanitized = value;
    if (field === "firstName" || field === "lastName") sanitized = value.replace(/[^a-zA-Z\s'\-]/g, "").slice(0, 50);
    if (field === "passport") sanitized = value.replace(/[^a-zA-Z0-9]/g, "").toUpperCase().slice(0, 20);
    if (field === "phone") sanitized = value.replace(/[^0-9+]/g, "").slice(0, 15);
    if (field === "email") sanitized = value.slice(0, 254);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setServerError("Booking failed. Please check your connection and try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={s.layout}>
              ...
              <div key={i} style={{ ...s.formCard, marginBottom: "12px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "100px 1fr 1fr", gap: "12px", marginBottom: "12px" }}>
                  <div style={s.fieldWrap}>
                    ...
                    <select value={p.title} onChange={(e) => updatePassenger(i, "title", e.target.value)} style={s.select}>
                      {TITLES.map((t) => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </div>
                  <div style={s.fieldWrap}>
                    <label style={s.label}>First name</label>
                    <input value={p.firstName} onChange={(e) => updatePassenger(i, "firstName", e.target.value)} onFocus={() => setFocused(`${i}_firstName`)} onBlur={() => setFocused(null)} style={getInputStyle(`${i}_firstName`)} placeholder="As on passport" maxLength={50} />
                    {errors[`${i}_firstName`] && <p style={s.errorMsg}>{errors[`${i}_firstName`]}</p>}
                  </div>
                  <div style={s.fieldWrap}>
                    <label style={s.label}>Last name</label>
                    <input value={p.lastName} onChange={(e) => updatePassenger(i, "lastName", e.target.value)} onFocus={() => setFocused(`${i}_lastName`)} onBlur={() => setFocused(null)} style={getInputStyle(`${i}_lastName`)} placeholder="As on passport" maxLength={50} />
                ...
                <div style={s.grid2}>
                  <div style={s.fieldWrap}>
                    ...
                    <input type="email" value={p.email} onChange={(e) => updatePassenger(i, "email", e.target.value)} onFocus={() => setFocused(`${i}_email`)} onBlur={() => setFocused(null)} style={getInputStyle(`${i}_email`)} placeholder="email@example.com" maxLength={254} />
                    {errors[`${i}_email`] && <p style={s.errorMsg}>{errors[`${i}_email`]}</p>}
                  </div>
                  <div style={s.fieldWrap}>
                    <label style={s.label}>Phone</label>
                    <input type="tel" value={p.phone} onChange={(e) => updatePassenger(i, "phone", e.target.value)} onFocus={() => setFocused(`${i}_phone`)} onBlur={() => setFocused(null)} style={getInputStyle(`${i}_phone`)} placeholder="+201234567890" maxLength={15} />
                ...
                <div style={s.grid1}>
                  <div style={s.fieldWrap}>
                    ...
                    <p style={{ fontSize: "11px", color: "#3a4055", marginTop: "4px" }}>Encrypted end-to-end. Never stored in plain text.</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div>
            <div style={s.sideCard}>
              ...
              <div style={s.divider} />
              <div style={s.priceLine}><span style={s.priceLabel}>Base fare × {flight.passengers}</span><span style={s.priceVal}>${totalPrice}</span></div>
              <div style={s.priceLine}><span style={s.priceLabel}>Taxes & fees</span><span style={s.priceVal}>${taxes}</span></div>
              <div style={s.priceTotalLine}><span style={s.priceTotalLabel}>Total</span><span style={s.priceTotalVal}>${grandTotal}</span></div>
            </div>
